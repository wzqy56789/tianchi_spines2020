...
def extract_bboxes(mask):
    ...
    for i in range(mask.shape[-1]):
        m = mask[:, :, i]
        # Bounding box.   any() 函数用于判断给定的可迭代参数 iterable 是否全部为 False，则返回 False，如果有一个为 True，则返回 True。
...
def resize_image(image, min_dim=None, max_dim=None, min_scale=None, mode="square"):
    ...
    if mode == "square":
        ...
        #padding = [(top_pad, bottom_pad), (left_pad, right_pad)]#只填充前两维。二维
        """
...
def resize_mask(mask, scale, padding, crop=None):
    ...
    if crop is not None:
        ...
        #image的padding是灰度图，二维的。mask三维，需要将padding补充成3维
        padding2=[padding[0],padding[1], (0, 0)]
        mask = np.pad(mask, padding2, mode='constant', constant_values=0)
    return mask
